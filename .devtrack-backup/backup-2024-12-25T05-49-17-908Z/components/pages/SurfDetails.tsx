...
export function sortInterests(
  ...
  const aIncluded = myInterests?.includes(parseInt(a));
  const bIncluded = myInterests?.includes(parseInt(b));
  if (aIncluded && !bIncluded) return -1;
  if (!aIncluded && bIncluded) return 1;
  return 0;
}

export default function SurfDetails({ 
  route, 
  navigation 
}: { 
  ...
  const [myData, setMyData] = useState<Profile | null>(null);
    ...
    const fetchUser = async () => {
      try {
        ...
        const { data, error } = await supabase
          .from("profiles_test")
          .select("id, name, age, interests, bio")
          .eq("id", id)
          .single();
